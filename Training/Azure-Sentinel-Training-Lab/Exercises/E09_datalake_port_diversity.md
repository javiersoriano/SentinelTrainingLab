# Exercise 9 — Data Lake vs Real-Time Detection

**Rule:** `[E7] [Palo Alto] Data Lake Port Diversity Anomaly`
**Deployed in:** `Artifacts/DetectionRules/rules.json`
**MITRE ATT&CK:** T1046 (Network Service Discovery)
**Difficulty:** Intermediate
**Prerequisites:** Complete [Exercise 6 — Data Lake KQL Jobs](./E06_datalake_kql_jobs.md) first (creates the `PaloAlto_ThreatSummary_KQL_CL` table used by this exercise)

---

## Objective

Understand the difference between **real-time log detection** (E2) and **data lake aggregated detection** (E7) by building a detection against pre-aggregated data. Learn when each approach is appropriate.
...
| Factor | Real-Time (E2) | Data Lake (E7) |
|---|---|---|
...
> **Key insight:** E2 (port scan) catches the scan as it happens. E7 (port diversity) catches the same pattern from aggregated data — useful for environments where real-time syslog isn't available or for retroactive hunting.
...
Compare this to E2, which must compute `dcount(DestinationPort)` from raw events.
...
The E7 threshold (`DistinctPorts > 10`) is lower than E2 (`> 20`) because data lake aggregation covers a wider time window. Try adjusting:
...
1. Enable both E2 and E7
2. Compare the alerts generated by each rule:
   - Which fired first?
