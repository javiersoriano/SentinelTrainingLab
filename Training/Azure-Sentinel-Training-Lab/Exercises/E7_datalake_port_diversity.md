...
Understand the difference between **real-time log detection** (E2) and **data lake aggregated detection** (E5) by building a detection against pre-aggregated data. Learn when each approach is appropriate.
...
| Factor | Real-Time (E2) | Data Lake (E5) |
|---|---|---|
...
> **Key insight:** E2 (port scan) catches the scan as it happens. E5 (port diversity) catches the same pattern from aggregated data — useful for environments where real-time syslog isn't available or for retroactive hunting.
...
The E5 threshold (`DistinctPorts > 10`) is lower than E2 (`> 20`) because data lake aggregation covers a wider time window. Try adjusting:
...
1. Enable both E2 and E5
2. Run an attack simulation: `python -m scripts.ingest --attack --connector paloalto`
